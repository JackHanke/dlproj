...
def policy_loss(pi: torch.Tensor, p_theta: torch.Tensor, epsilon: float = 1e-8) -> torch.Tensor:
    """
    ...
    Args:
        ...
        p_theta (torch.Tensor): Predicted policy from the neural network (probability distribution over actions).
        epsilon (float): Small value to prevent log(0) issues (default: 1e-8).

    Returns:
        torch.Tensor: Scalar tensor representing the policy loss.
    """
    return -torch.mean(torch.sum(pi * torch.log(p_theta + epsilon), dim=-1))  # Mean over batch
...
def combined_loss(
    ...
    Args:
        ...
        reg_weight (float): Weighting factor for L2 regularization (default: 1e-4).

    Returns:
        torch.Tensor: Scalar tensor representing the total loss.
    """
    # Compute individual losses
    p_loss = policy_loss(pi, p_theta)
    v_loss = value_loss(z, v_theta)

    # Compute L2 regularization loss
    reg_loss = sum(p.pow(2).sum() for p in p_theta) * reg_weight

    # Compute total loss with weights
    total_loss = policy_weight * p_loss + value_weight * v_loss + reg_loss
